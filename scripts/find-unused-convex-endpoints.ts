...
const DEV_ONLY_MODULES = new Set(["testing/testing", "testing/emails", "emails/index", "devAuth"]);

// Functions that are kept for future use or seeding (add here if needed)
// Note: Use api.xxx format even for internal functions (apiPath format)
const ALLOWED_UNUSED = new Set<string>([
  "api.web.subscribe", // WebSocket subscription endpoint for future use
  "api.emails.sendWelcomeEmail", // Email template ready for use
]);
