  ...
  useEffect(() => {
    if (profile && !isEditing) {
      setFormData({
        ...
        name: profile.name || betterAuthUser?.name || "",
        email: profile.email || betterAuthUser?.email || "",
        // For avatar, use Better-Auth image as fallback
        avatar: profile.image || betterAuthUser?.image || "",
      });
    }
  }, [profile, betterAuthUser, isEditing]);

  // Avatar URL is derived directly from profile.image
  const imageUrl = profile?.image || betterAuthUser?.image;
