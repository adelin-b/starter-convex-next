...
const SIDEBAR_COOKIE_MAX_AGE = 60 * 60 * 24 * COOKIE_MAX_AGE_DAYS;
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (newValue: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: Sidebar state is non-sensitive UI preference following Shadcn pattern
    ...
    return (
      <Sheet onOpenChange={setOpenMobile} open={openMobile} {...props}>
        ...
        >
          ...
          <div className="flex size-full flex-col pt-safe">{children}</div>
        </SheetContent>
      </Sheet>
    );
  }
...
// Skeleton width range constants (50% to 90%)
...
function SidebarMenuSkeleton({
  ...
  const width = React.useMemo(
    () => `${Math.floor(Math.random() * SKELETON_WIDTH_RANGE) + SKELETON_MIN_WIDTH_PERCENT}%`,
    [],
  );
