...
export const ShouldSelectOption: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("open and select item", async () => {
      ...
      await userEvent.click(await canvasBody.findByRole("option", { name: BANANA_PATTERN }));
      expect(select).toHaveTextContent("Banana");
    });

    await step("verify the selected option", async () => {
      await userEvent.click(select);
      expect(await canvasBody.findByRole("option", { name: BANANA_PATTERN })).toHaveAttribute(
        "data-state",
        "checked",
      );
      await userEvent.click(await canvasBody.findByRole("option", { name: BANANA_PATTERN }));
