...
const FILE_MENU_PATTERN = /file/i;
const EXPECTED_FILE_MENU_ITEMS = 5;
...
export const ShouldOpenClose: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("open the menubar", async () => {
      await userEvent.click(await canvasBody.findByRole("menuitem", { name: FILE_MENU_PATTERN }));
      expect(await canvasBody.findByRole("menu")).toBeInTheDocument();
    });

    const items = await canvasBody.findAllByRole("menuitem");
    expect(items).toHaveLength(EXPECTED_FILE_MENU_ITEMS);
